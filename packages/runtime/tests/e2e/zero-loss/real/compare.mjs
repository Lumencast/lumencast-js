...
const stats = await page.evaluate(() => {
  ...
  const grid = cell
    .map((row) => row.map((n) => ((100 * n) / cellN).toFixed(1).padStart(5)).join(" "))
    .join("\n");
...
const bunFields = (n, absX, absY) => ({
  ...
  absX,
  absY,
  w: n.size ? n.size.w : undefined,
  h: n.size ? n.size.h : undefined,
  rot: n.rotation ?? 0,
  cr: n.cornerRadius,
  opacity: n.opacity ?? 1,
  visible: n.visible !== false,
...
function compare(fig, bun, path, accX, accY) {
  ...
  if (f.absX !== undefined && f.aabbW !== undefined && b.w !== undefined) {
    const fcx = f.absX + f.aabbW / 2,
      bcx = b.absX + b.w / 2;
    const fcy = f.absY + f.aabbH / 2,
      bcy = b.absY + b.h / 2;
  ...
  const fw = f.sizeW ?? f.aabbW,
    fh = f.sizeH ?? f.aabbH;
  if (b.w !== undefined && fw !== undefined && Math.abs(fw - b.w) >= 1)
    note(id, "w", r3(fw), r3(b.w));
  if (b.h !== undefined && fh !== undefined && Math.abs(fh - b.h) >= 1)
    note(id, "h", r3(fh), r3(b.h));
  if (Math.abs((f.rot || 0) - (b.rot || 0)) >= 0.1) note(id, "rot", f.rot, b.rot);
  if ((f.cr ?? 0) !== (b.cr ?? 0) && Math.abs((f.cr ?? 0) - (b.cr ?? 0)) >= 0.5)
    note(id, "cornerRadius", f.cr, b.cr);
  if (Math.abs((f.opacity ?? 1) - (b.opacity ?? 1)) >= 0.01)
    note(id, "opacity", f.opacity, b.opacity);
  ...
  if (b.kind === "shape" && n_isPath(bun)) {
    if (f.path && b.path && f.path.replace(/\s+/g, "") !== b.path.replace(/\s+/g, ""))
      note(
        id,
        "pathData",
        "≠ (" + f.path.length + " chars)",
        "(" + (b.path ? b.path.length : 0) + " chars)",
      );
  ...
  for (const fc of figCh) {
    let bi = bunCh.findIndex(
      (c, i) =>
        !used.has(i) &&
        ((c.metadata && c.metadata.figma && c.metadata.figma.layerName) || "") === fc.name,
    );
    if (bi === -1) {
      gaps.push(`${id} | child MISSING in bundle: "${fc.name}" (${fc.type})`);
      continue;
    }
    used.add(bi);
  ...
  for (let i = 0; i < bunCh.length; i++)
    if (!used.has(i))
      gaps.push(
        `${id} | extra in bundle: "${(bunCh[i].metadata && bunCh[i].metadata.figma && bunCh[i].metadata.figma.layerName) || "?"}" (${bunCh[i].kind})`,
      );
}
...
writeFileSync(
  `${OUT}/06-numeric-gaps.txt`,
  `${gaps.length} gaps (≥1px / ≥0.1° / pathData / presence) vs Figma 817:3\n\n${gaps.join("\n")}\n`,
);
